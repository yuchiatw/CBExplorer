...
class GNet(nn.Module):
    def __init__(self,
        ...
        # adding +2 manually since conv-transpose2 block ends after +2 (check with below print statements)
        self.midpoint = self.num_layers // 2 + 2

        print('')
        print(self.main[:self.midpoint])
        print('')
        print(self.main[self.midpoint:])
    ...
    def forward_part1(self, input):
        x = input.view(-1, input.size(1), 1, 1)
        x = self.formatLayer(x)

        for idx in range(self.midpoint):
            x = self.main[idx](x)
        
        return x

    def forward_part2(self, input):
        x = input
        for idx in range(self.midpoint, self.num_layers):
            x = self.main[idx](x)
